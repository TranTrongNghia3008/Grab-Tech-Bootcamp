...
const tabs = [
  { label: "Overview", icon: <FiGrid className="mr-2" /> },
...
export default function ProjectDetail() {
  ...
  const { projectName, isModel } = state;
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState("Overview");
  const [showHelp, setShowHelp] = useState(false);

  useEffect(() => {
    document.title = "DataMate - Project Detail";
  }, []);
  ...
  const handleTabClick = (tabLabel) => {
    if (!isTargetFeatureSelected && !isModel && ["Modeling", "Export"].includes(tabLabel)) {
      ...
      <div className="flex flex-wrap items-center justify-between gap-y-2 mb-2 border-b border-[#CDEBD5] p-4 bg-[#00843D] sticky top-0 z-100">
        ...
        <div className="flex items-center gap-4">
          <div className="inline-flex bg-[#E4F3E9] rounded-xl shadow-inner px-2 py-1 overflow-x-auto">
            {tabs.map((tab) => {
              const isDisabled = !isTargetFeatureSelected && !isModel && ["Modeling", "Export"].includes(tab.label);
              return (
                <button
                  key={tab.label}
          ...
          {showHelp && <HelpInfoModal onClose={() => setShowHelp(false)} />}
        </div>
      </div>
