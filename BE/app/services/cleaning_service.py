...
def run_cleaning_job(job_id: int, db):
    ...
    automate_cleaning_by_task(
        input_csv=original,
        output_csv=output,
        remove_duplicates=job.config.get('drop_duplicates', False),
        handle_missing_values='impute_missing' in (job.config or {}),
        smooth_noisy_data=False,
        handle_outliers='filter_outliers' in (job.config or {}),
        reduce_cardinality=False,
        encode_categorical_values='enforce_types' in (job.config or {}),
    ...
    job = update_cleaning_job(db, job, status='done', results=results)
    return job
